/*
 Programming Principles and Practice in C++, 2nd ed. Chapter 7  Exercise 3
Allow 'const a = 5' to set constant values
/*
...
// set the Variable named s to d
{
    for (int i = 0; i <= names.size(); ++i)
        if (names[i].name == s)
        {
            if (names[i].constant) error("Constant values can not be given a new value");
            names[i].value = d;
            return;
        }
...
double primary()
{
    ...
    case name:
    {
        ...
        if (next_token.kind == '=')
        {
            if (!is_declared(t.name)) error ("Can't alter value of undeclared variable. Use 'let x = 6' to declare a new variable.");
